...
export const ArchiveView: React.FC = () => {
  const { archivedProposals, isLoading, refundToCreator, isRefundingPending, accountAddress } = useDeVoteContract();
      ...
      {archivedProposals.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {archivedProposals.map((prop) => {
            const isCreator = accountAddress?.toLowerCase() === prop.creator.toLowerCase();
            const remainingPool = prop.rewardAmount - prop.claimedAmount;
            const hasRemaining = remainingPool > 0n;
            ...
            return (
              <div key={String(prop.id)} className="bg-neutral-800 rounded-xl p-6 border border-neutral-700 flex flex-col justify-between opacity-80 grayscale-[0.3] hover:grayscale-0 transition-all">
                ...
                <div className="mt-4">
                  {isCreator && hasRemaining && (
                    <button
                      onClick={() => handleRefund(prop.id)}
                      disabled={isRefundingPending(prop.id)}
                      className="w-full py-2 mb-4 text-xs font-bold rounded-lg bg-[#a5c2ff] text-black hover:bg-[#8eafef] disabled:opacity-50 transition-all shadow-lg shadow-blue-500/20"
                    >
                      {isRefundingPending(prop.id) ? (
                        <Spinner className="w-4 h-4 mx-auto" />
                      ) : (
                        `Claim Remaining: ${(Number(remainingPool) / 10 ** 18).toFixed(2)} HASH`
                      )}
                    </button>
                  )}
                </div>
                
                <div className="mt-auto pt-4 border-t border-neutral-700/50 flex justify-between items-center text-[10px] text-neutral-500 font-medium">
